...
const API_BASE_URL = `${API_BASE}/product`;
const IMAGE_API_URL = `${API_BASE}/images`;
...
export const ProductApi = {
  ...
  fetchProducts: async (page = 0, size = 5, category?: string): Promise<ProductData> => {
    let url = `${API_BASE_URL}/getAll?page=${page}&size=${size}&sort=createdAt,desc`;
  ...
  patchProduct: async (id: string, updates: Partial<Product>): Promise<Product> => {
    try {
      ...
      console.error(`Error patching product ${id}:`, error.message || error);
      throw error;
    }
  },
